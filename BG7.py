...
def bigrams_in_order_and_consecutive(word, bigrams):
    ...
    word_len = len(word_bigrams)
    bigram_len = len(bigrams)

    for i in range(word_len - bigram_len + 1):
        if word_bigrams[i:i+bigram_len] == list(bigrams):
            return True
    return False
...
if __name__ == "__main__":
    ...
    bigrams = generate_bigrams("proportional")
    print("Bigrams:", bigrams)
    unique_bigrams = list(set(bigrams))

    # Sort the unique bigrams and truncate to first 5
    unique_bigrams.sort()
    truncated_bigrams = unique_bigrams[:5]
    
    # Create a tuple of the truncated bigrams
    bigram_tuple = tuple(truncated_bigrams)
